...
// Pojedyncza kartka (front zdzieraka). Zagięte rogi: prawy = następny dzień, lewy = poprzedni.
function Sheet({ d, onNext, onPrev, hint = false }: { d: DayData; onNext?: () => void; onPrev?: () => void; hint?: boolean }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_20px_60px_rgba(0,56,144,0.12)]">
      ...
      <div className="flex min-h-[500px] flex-col px-6 pb-6 pt-3 text-center">
        <div className="flex items-start justify-between text-[11px] font-medium text-slate-500">
          <div className="text-left leading-tight">
            <div>{d.doy}. dzień roku</div>
            <div>zostało {d.remaining} dni</div>
          </div>
        ...
        {d.holiday && (
          <div className="mt-2 inline-block rounded-full bg-red-50 px-3 py-1 text-xs font-semibold text-red-600">
            {d.holiday} — dzień wolny
          </div>
        )}

        {d.names.length > 0 && (
          <p className="mt-3 line-clamp-2 text-sm text-slate-700">
            <span className="text-slate-500">Imieniny: </span>
        ...
        <div className="mt-auto pt-5">
          ...
          >
            Otwórz pełną kartkę <span aria-hidden>→</span>
          </Link>
        </div>
      </div>

      {/* Prawy zagięty róg — następny dzień (zrywamy kartkę do góry) */}
      {onNext && (
        <button
          type="button"
          onClick={onNext}
          aria-label="Zerwij kartkę — pokaż następny dzień"
      ...
      {/* Lewy zagięty róg — poprzedni dzień (kartka wraca) */}
      {onPrev && (
        <button
          type="button"
          onClick={onPrev}
          aria-label="Cofnij kartkę — pokaż poprzedni dzień"
...
export default function TearOffCalendarClient({ days, startIndex = 0 }: { days: DayData[]; startIndex?: number }) {
  const [idx, setIdx] = useState(startIndex);
  const [anim, setAnim] = useState<null | "next" | "prev">(null);
  const [hint, setHint] = useState(true); // podpowiedź „przewróć kartkę" tylko na start
  ...
  // Warstwa górna (animowana) i dolna (odsłaniana/statyczna) zależą od kierunku.
  // next: bieżąca kartka zrywa się do góry, odsłaniając następny dzień pod spodem.
  // prev: poprzedni dzień „wraca" z góry na wierzch bieżącej kartki.
  ...
  return (
    <div className="relative mx-auto w-full max-w-md" style={{ perspective: "1400px" }}>
      ...
      <div className="relative">
        {/* warstwa dolna — odsłaniana podczas animacji */}
        <div className="absolute inset-0" aria-hidden={!anim}>
          <Sheet d={bottom} />
        </div>
        {/* warstwa górna — animowana; w spoczynku ma aktywne rogi */}
        <div
